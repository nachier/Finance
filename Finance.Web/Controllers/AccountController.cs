...
namespace Finance.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var file = HttpContext.Request.Files["ImageUrl"];
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        RoleManager.Create(new ApplicationRole { Id = 1, Name = "Normal", Description = "普通用户" });
                    }
                    var IsSuccessAddToRole = await UserManager.AddToRoleAsync(user.Id, "Normal");
                    if(!IsSuccessAddToRole.Succeeded)
                    {
                        AddErrors(IsSuccessAddToRole);
                    }
                    else
                    {
